...
local function file_browser()
  ...
  local buf_dir = buf_path == "" and vim.fn.getcwd() or vim.fn.fnamemodify(buf_path, ":h")
  ...
  local finder = function(opts, ctx)
    ...
    local items = {{file = "", text = "", cwd = cwd, _path = current_dir .. "/", type = "directory"}}
    for i, v in ipairs(files) do
      -- local path = current_dir .. "/" .. v
      local path = current_dir == "/" and current_dir..v or current_dir .. "/" .. v
      ...
      items[#items + 1] = {file = v, text = v, cwd = cwd, _path = path, type = type, is_link = is_link}
    end
  ...
  local format = function(item, picker)
    ...
    if item.is_link then
      ...
      local highlight = not item.type and "SnacksPickerLinkBroken" or "SnacksPickerPathHidden"
    ...
    if item.type == "directory" then
      ...
      if ret[2][1] == "" then
        ...
        local path_print =  ((rel_path and " ~/"..rel_path) or " " .. picker:cwd())
  ...
  local preview = function(ctx)
    ...
    if item.type == "directory" then
      ...
      for i, entry in pairs(entries) do
        ...
        vim.api.nvim_buf_set_extmark(ctx.buf,ns, i-1,0,{virt_text = line})
      end
